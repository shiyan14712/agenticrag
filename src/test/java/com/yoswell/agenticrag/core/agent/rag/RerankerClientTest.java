...
class RerankerClientTest {
    ...
    void rerankFallsBackToOriginalOrderWhenEndpointFails() {
        ...
        List<RetrievedChunk> original = List.of(
                new RetrievedChunk("chk-1", "doc-1", "doc-1.txt", "tenant-a", "kb-1", List.of("ROLE_USER"), 0, "alpha", 0.9),
                new RetrievedChunk("chk-2", "doc-2", "doc-2.txt", "tenant-a", "kb-1", List.of("ROLE_USER"), 1, "beta", 0.8)
        );

        List<RetrievedChunk> reranked = client.rerank("query", original);
